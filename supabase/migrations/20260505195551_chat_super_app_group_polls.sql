...
CREATE POLICY "Members can read group polls" ON public.group_polls FOR SELECT USING (EXISTS (SELECT 1 FROM public.chat_group_members m WHERE m.group_id = group_polls.group_id AND m.user_id = auth.uid()));
DROP POLICY IF EXISTS "Members can create polls in their groups" ON public.group_polls;
CREATE POLICY "Members can create polls in their groups" ON public.group_polls FOR INSERT WITH CHECK (auth.uid() = created_by AND EXISTS (SELECT 1 FROM public.chat_group_members m WHERE m.group_id = group_polls.group_id AND m.user_id = auth.uid()));
