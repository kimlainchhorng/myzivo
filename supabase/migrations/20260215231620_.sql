...
-- Add matching regions for all new cities that don't have one
...
-- Add matching eats_zones for all new cities that don't have one
...
SELECT
  UPPER(LEFT(REPLACE(REPLACE(c.name, ' ', ''), '''', ''), 5)) || c.state,
  c.name, c.id,
  CASE c.state
