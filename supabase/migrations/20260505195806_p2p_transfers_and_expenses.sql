...
CREATE POLICY "Members read group expenses" ON public.group_expenses FOR SELECT USING (EXISTS (SELECT 1 FROM public.chat_group_members m WHERE m.group_id = group_expenses.group_id AND m.user_id = auth.uid()));
DROP POLICY IF EXISTS "Members create group expenses" ON public.group_expenses;
CREATE POLICY "Members create group expenses" ON public.group_expenses FOR INSERT WITH CHECK (auth.uid() = paid_by AND EXISTS (SELECT 1 FROM public.chat_group_members m WHERE m.group_id = group_expenses.group_id AND m.user_id = auth.uid()));
...
CREATE POLICY "Users read shares involving them" ON public.group_expense_shares FOR SELECT USING (auth.uid() = user_id OR EXISTS (SELECT 1 FROM public.group_expenses e JOIN public.chat_group_members m ON m.group_id = e.group_id WHERE e.id = group_expense_shares.expense_id AND m.user_id = auth.uid()));
DROP POLICY IF EXISTS "Expense owners create shares" ON public.group_expense_shares;
CREATE POLICY "Expense owners create shares" ON public.group_expense_shares FOR INSERT WITH CHECK (EXISTS (SELECT 1 FROM public.group_expenses e WHERE e.id = group_expense_shares.expense_id AND e.paid_by = auth.uid()));
