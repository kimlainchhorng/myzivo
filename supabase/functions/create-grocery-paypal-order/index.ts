 ...
 * ----------------------------
 * Creates a PayPal Order for a grocery shopping_orders row. Returns approve_url.
 * Buyer is redirected to PayPal; on approval they come back to ?grocery_paypal_return=...
 * and capture-grocery-paypal-order finalises the payment.
 */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const orderRes = await fetch(`${PAYPAL_BASE}/v2/checkout/orders`, {
      method: "POST",
      headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json", "PayPal-Request-Id": `grocery-${order_id}-${amount_cents}` },
      body: JSON.stringify({
        intent: "CAPTURE",
        purchase_units: [{
          reference_id: order_id, custom_id: order_id,
          description: "ZIVO Grocery order",
        ...
        application_context: {
          ...
          user_action: "PAY_NOW",
          return_url: return_url || `${req.headers.get("origin") ?? ""}/grocery/orders`,
          cancel_url: cancel_url || `${req.headers.get("origin") ?? ""}/grocery/orders`,
        },
      }),
    });
    ...
    console.error("[create-grocery-paypal-order]", msg);
