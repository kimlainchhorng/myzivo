      ...
      <div className="flex items-center justify-between text-sm text-white max-w-md mx-auto">
        ...
        {onRetry && error.isRetryable && (
          <button
            onClick={onRetry}
            disabled={isRetrying}
            className="px-3 py-1 bg-white/20 hover:bg-white/30 active:scale-90 rounded-full text-xs font-medium transition-all duration-200 flex items-center gap-1 disabled:opacity-50 touch-manipulation"
          >
            {isRetrying ? (
              <>
