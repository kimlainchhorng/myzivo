...
interface CarStatsBarProps {
  className?: string;
}

export default function CarStatsBar({ className }: CarStatsBarProps) {
  const stats = [
    { 
      icon: Car, 
      value: 25000, 
  ...
  const trustBadges = [
    ...
    { icon: CheckCircle, label: "Verified Rentals", color: "text-violet-500" },
    { icon: Award, label: "Best Rates", color: "text-purple-500" },
    { icon: Clock, label: "24/7 Roadside", color: "text-sky-500" },
  ];
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-6 sm:mb-8">
          {stats.map((stat, index) => (
            <div
              key={stat.label}
              className="flex flex-col items-center p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-card/50 backdrop-blur-xl border border-border/50 hover:border-violet-500/30 transition-all duration-200 group animate-in fade-in slide-in-from-bottom-4 touch-manipulation"
