/**
 * EatsAutoPayoutLedger
 * --------------------
 * Read-only view of `eats_payout_ledger` for a restaurant. Shows every Stripe
 * Connect auto-transfer (and any reversals on refund) so the restaurant can
 * see exactly what landed in their bank account, when, and why.
 *
 * Auto-transfers are created by stripe-webhook on payment_intent.succeeded for
 * Stripe-paid food_orders whose restaurant has Connect onboarded and
 * auto_payout_enabled = true. Reversals fire on refund/cancel.
 */
...
export default function EatsAutoPayoutLedger({ restaurantId }: Props) {
  const { data: rows = [], isLoading, error } = useQuery({
    queryKey: ["eats-payout-ledger", restaurantId],
    queryFn: async (): Promise<LedgerRow[]> => {
      const { data: ledger, error: lErr } = await (supabase
        .from("eats_payout_ledger") as any)
        .select("*")
      ...
      const { data: orders } = await (supabase
        ...
        .in("id", ids);
      const map = new Map<string, string>((orders || []).map((r: any) => [r.id, r.tracking_code]));
      return list.map((r) => ({ ...r, tracking_code: map.get(r.order_id) ?? null }));
    },
    ...
    <div className="rounded-lg border border-border bg-card p-4 space-y-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">
            Stripe Connect transfers we send to your bank when customers pay by card. Reversals run automatically on refund.
          </p>
        </div>
        {rows.length > 0 && (
      ...
      {isLoading ? (
        ...
        <p className="text-xs text-muted-foreground py-3">
          No automatic transfers yet. Once a customer pays by card and your Stripe account is connected, your share will land here automatically.
        </p>
      ) : (
          ...
          <table className="w-full text-xs">
            <thead className="text-muted-foreground border-b border-border/60">
              <tr>
                ...
                <th className="text-left font-medium pb-2 pr-3">Order</th>
              ...
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className="align-top">
                    ...
                    <td className="py-2 pr-3 whitespace-nowrap font-mono">{r.tracking_code ?? r.order_id.slice(0, 8)}</td>
