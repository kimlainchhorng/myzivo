  ...
  return (
    <SectionShell title="Experiences & Tours" subtitle="Resort activities, tours, cruises, and destination experiences tied to reservations." icon={Palmtree}>
      {isLoading ? <LoadingPanel /> : <>
        ...
        <AddonList addons={experiences} emptyTitle="Experience catalog is ready to fill" emptyBody="Add snorkeling tours, island hopping, fishing trips, sunset cruises, and private destination packages guests can request." />
        <NextActions actions={[{ label: "Add tour add-ons", tab: "lodge-rooms", hint: "Create guest-bookable experiences." }, { label: "Update nearby attractions", tab: "lodge-property", hint: "Show nearby beaches, marinas, and landmarks." }, { label: "Check guest requests", tab: "lodge-guest-requests", hint: "Review experience add-on requests." }]} />
      </>}
    </SectionShell>
  );
}
