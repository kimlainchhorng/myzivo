  ...
  return (
    <SectionShell title="Spa & Wellness" subtitle="Massage, spa, yoga, gym, sauna, pool, and wellness services for resort guests." icon={HeartPulse}>
      {isLoading ? <LoadingPanel /> : <>
        ...
        <AddonList addons={wellness} emptyTitle="Wellness catalog is ready to fill" emptyBody="Add spa massage, couples massage, yoga sessions, sauna access, pool passes, or wellness packages to room add-ons." />
        <NextActions actions={[{ label: "Add spa packages", tab: "lodge-rooms", hint: "Create wellness add-ons guests can request." }, { label: "Update facilities", tab: "lodge-property", hint: "Mark spa, pool, gym, and sauna facilities." }, { label: "Review service requests", tab: "lodge-guest-requests", hint: "Track wellness add-ons in request workflows." }]} />
      </>}
    </SectionShell>
  );
}
