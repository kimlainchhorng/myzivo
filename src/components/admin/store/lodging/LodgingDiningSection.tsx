  ...
  return (
    <SectionShell title="Dining & Meal Plans" subtitle="Breakfast, board plans, room service, and food packages guests can add to stays." icon={Utensils}>
      {isLoading ? <LoadingPanel /> : <>
        ...
        <AddonList addons={dining} emptyTitle="Dining catalog is ready to fill" emptyBody="Add breakfast buffet, board plans, room service, kids meals, floating breakfast, and romantic dinner packages from Rooms & Rates." />
        <NextActions actions={[{ label: "Add dining packages", tab: "lodge-rooms", hint: "Attach meal add-ons to guest rooms." }, { label: "Edit meal plans", tab: "lodge-property", hint: "Set property-level board options." }, { label: "Review guest requests", tab: "lodge-guest-requests", hint: "Track dining requests and add-on follow-up." }]} />
      </>}
    </SectionShell>
  );
}
