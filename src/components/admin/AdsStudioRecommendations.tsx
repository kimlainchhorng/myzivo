      ...
      <CardContent>
        {loading ? <Skeleton className="h-20 w-full" /> : recs.length === 0 ? (
          ...
          <div className="space-y-2">
            {recs.map((r) => (
              <div key={r.id} className="p-2.5 rounded-lg border border-border/60 bg-muted/30 space-y-2">
                <div className="flex items-start gap-2 flex-wrap">
                  <Badge className={`text-[10px] shrink-0 ${TYPE_COLOR[r.recommendation_type] || ""}`}>
                    {r.recommendation_type.replace("_", " ")}
                  </Badge>
                  <span className="text-xs font-semibold flex-1 min-w-0 break-words">{r.title}</span>
                </div>
                <p className="text-[11px] text-muted-foreground leading-relaxed break-words">{r.body}</p>
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  {r.estimated_impact && (
                    <span className="text-[10px] text-emerald-600 font-medium break-words">📈 {r.estimated_impact}</span>
                  )}
                  <div className="ml-auto flex gap-1 shrink-0">
                    <Button size="sm" variant="ghost" className="h-8 sm:h-6 text-[11px] sm:text-[10px] px-2.5 sm:px-2" onClick={() => respond(r.id, "dismissed")} disabled={busy === r.id}>
                      <X className="h-3 w-3 mr-0.5" /> Dismiss
                    </Button>
                    <Button size="sm" className="h-8 sm:h-6 text-[11px] sm:text-[10px] px-2.5 sm:px-2" onClick={() => respond(r.id, "accepted")} disabled={busy === r.id}>
