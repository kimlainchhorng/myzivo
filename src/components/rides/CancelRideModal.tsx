...
const CUSTOMER_REASONS: CancelReason[] = [
  { id: "changed_mind", label: "Changed my mind", icon: Ban, fee: true },
  { id: "wrong_pickup", label: "Wrong pickup location", icon: MapPin, fee: false },
  { id: "driver_too_far", label: "Driver is too far away", icon: Clock, fee: false },
  { id: "found_other_ride", label: "Found another ride", icon: Car, fee: true },
  { id: "plans_changed", label: "Plans changed", icon: AlertTriangle, fee: true },
  { id: "safety_concern", label: "Safety concern", icon: Shield, fee: false },
  { id: "price_too_high", label: "Price is too high", icon: CircleDollarSign, fee: true },
  { id: "long_wait", label: "Wait time too long", icon: Timer, fee: false },
];

const DRIVER_REASONS: CancelReason[] = [
  { id: "no_show", label: "Customer no-show (5+ min wait)", icon: UserX, fee: true, description: "You'll receive a cancellation fee" },
  { id: "passenger_mismatch", label: "More passengers than booked", icon: Users, fee: true, description: "Customer booked fewer seats than needed" },
  { id: "unsafe_pickup", label: "Unsafe pickup location", icon: Shield, fee: false },
  { id: "wrong_address", label: "Customer gave wrong address", icon: MapPin, fee: false },
  { id: "vehicle_issue", label: "Vehicle issue / breakdown", icon: Car, fee: false },
  { id: "emergency", label: "Personal emergency", icon: AlertTriangle, fee: false },
  { id: "excessive_luggage", label: "Excessive luggage / items", icon: AlertTriangle, fee: true, description: "Items exceed vehicle capacity" },
  { id: "intoxicated_rider", label: "Rider appears intoxicated", icon: Shield, fee: true, description: "Safety-related cancellation" },
];
            ...
            <div className="px-5 pb-6">
              ...
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-foreground">
                  {role === "customer" ? "Cancel ride" : "Cancel trip"}
                </h3>
              ...
              {role === "customer" && tripPhase !== "searching" && (
                <div className="rounded-xl bg-amber-500/10 border border-amber-500/20 p-3 mb-4 flex items-start gap-2.5">
                  ...
                  <p className="text-xs text-amber-700 dark:text-amber-400">
                    A cancellation fee may apply depending on the reason and timing.
                  </p>
                </div>
              )}
                ...
                {reasons.map((reason) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <span className="text-sm font-semibold text-foreground block">{reason.label}</span>
                      {reason.description && (
            ...
            <div className="px-5 pb-6">
              ...
              <div className="text-center mb-5">
                ...
                <h3 className="text-lg font-bold text-foreground mb-1">
                  {role === "customer" ? "Cancel this ride?" : "Cancel this trip?"}
                </h3>
              ...
              <div className="rounded-xl border border-border/30 p-4 mb-4 space-y-2">
                {fee > 0 ? (
                  <>
                    ...
                    {role === "customer" && (
                      <p className="text-[11px] text-muted-foreground">
                        This fee compensates the driver for their time and distance traveled.
                      </p>
                    )}
                    {role === "driver" && (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-emerald-600 dark:text-emerald-400 font-medium">No cancellation fee</span>
                  </div>
                )}
