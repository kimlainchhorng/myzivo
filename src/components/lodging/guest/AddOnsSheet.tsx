...
import { Minus, Plus, ShoppingBag, CreditCard, Loader2 } from "lucide-react";
...
export default function AddOnsSheet({ open, onOpenChange, reservationId, addons, nights, guests, onPurchased }: Props) {
  ...
  const selected = useMemo(() => Object.entries(qty).filter(([, count]) => count > 0), [qty]);
  const total = useMemo(
  ...
  const purchase = async () => {
    ...
    setLoading(false);
    if (error || data?.error) {
      toast.error(data?.error === "no_saved_payment_method" ? "No saved payment method found. Add a card in Wallet first." : data?.error || error?.message || "Could not charge add-ons");
      return;
    }
        ...
        <div className="mt-6 space-y-4">
          {!addons.length ? (
            ...
            addons.map((addon) => {
              ...
              const unit = String(addon.unit || addon.pricing_unit || "per_stay");
              return (
                <div key={id} className="rounded-lg border bg-card p-4 space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => setQty((q) => ({ ...q, [id]: Math.max(0, count - 1) }))}><Minus className="h-3.5 w-3.5" /></Button>
                      <span className="w-6 text-center text-sm font-semibold">{count}</span>
                      <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => setQty((q) => ({ ...q, [id]: count + 1 }))}><Plus className="h-3.5 w-3.5" /></Button>
                    </div>
                  </div>
          ...
          <div className="flex gap-2 sticky bottom-0 bg-background pt-3">
            ...
            <Button className="flex-1" disabled={!selected.length || loading} onClick={purchase}>
