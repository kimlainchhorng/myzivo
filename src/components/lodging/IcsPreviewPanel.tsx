 ...
 * Lets the guest tweak check-in/out times and address, then downloads via buildBookingIcs.
 */
import { useMemo, useState } from "react";
import { CalendarPlus, RotateCcw, MapPin, Clock } from "lucide-react";
    ...
    <div className="rounded-xl border border-border bg-card p-3 space-y-3">
      ...
      <div className="rounded-lg bg-muted/40 p-2 text-[11px] space-y-1">
        <div className="flex items-start gap-1.5">
          ...
          <div>
            <p className="font-semibold">Check-in: {formatPreview(defaults.checkIn, ciTime)}</p>
            {address && <p className="text-muted-foreground flex items-start gap-1"><MapPin className="h-2.5 w-2.5 mt-0.5 shrink-0" />{address}</p>}
          </div>
        </div>
        <div className="flex items-start gap-1.5">
          <Clock className="h-3 w-3 mt-0.5 text-amber-600 shrink-0" />
          <div>
            <p className="font-semibold">Check-out: {formatPreview(defaults.checkOut, coTime)}</p>
          </div>
        </div>
      </div>
