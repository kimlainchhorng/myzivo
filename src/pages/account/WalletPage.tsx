 ...
 * Real data only: Stripe cards, Supabase wallet transactions/credits
 */
import { useState } from "react";
import { ArrowLeft, Wallet, CreditCard, Star, Trash2, Plus, Shield, BarChart3, Users, Tag, Gift, Trophy, ExternalLink, TrendingUp, Clock } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { Button } from "@/components/ui/button";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Badge } from "@/components/ui/badge";
import { useCustomerWallet, type WalletTransaction } from "@/hooks/useCustomerWallet";
import { useStripePaymentMethods, useDeleteStripeCard, useSetDefaultStripeCard, type StripeCard } from "@/hooks/useStripePaymentMethods";
...
export default function WalletPage() {
  ...
  // Real data hooks
  const { balanceDollars, lifetimeEarnedDollars, transactions: creditTx, isLoading: walletLoading } = useCustomerWallet();
      ...
      <div className="px-6 py-6 space-y-6">
        ...
        >
          ...
          <div className="flex gap-8 mt-6 pt-4 border-t border-white/20">
            <div>
              <p className="text-[10px] text-white/60 uppercase tracking-wider">Total Spent</p>
              <p className="font-bold text-lg">{summaryLoading ? "..." : `$${totalSpent.toFixed(2)}`}</p>
            </div>
            <div>
                ...
                {stripeCards.map((card) => (
                  <div key={card.id} className="bg-card border border-border/50 rounded-xl p-4 flex items-center gap-3">
                    ...
                    <button
                      ...
                      title={card.is_default ? "Default card" : "Set as default"}
                    >
                      <Star className="w-4 h-4" fill={card.is_default ? "currentColor" : "none"} />
                    </button>
                    <button
                      onClick={() => {
                        if (confirm("Remove this card?")) deleteCard.mutate(card.id);
                      }}
        ...
        {/* Financial Insights — real summary data */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-bold">Financial Insights</h2>
          </div>
          ...
          {summaryLoading ? (
            ...
            <div className="space-y-4">
              {/* Spending by service */}
              {summary && Object.keys(summary.spentByService).length > 0 ? (
                <div className="bg-card border border-border/50 rounded-2xl p-5">
                  <h3 className="font-bold text-sm mb-3">Spending by Service</h3>
                  <div className="space-y-3">
                    {Object.entries(summary.spentByService).map(([service, amount]) => {
                ...
                <div className="bg-card border border-border/50 rounded-2xl p-6 text-center">
                  ...
                  <p className="text-xs text-muted-foreground/60 mt-1">Insights will appear as you use ZIVO services</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-card border border-border/50 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold">${(summary?.availableCredits ?? 0).toFixed(2)}</p>
                  <p className="text-[11px] text-muted-foreground mt-1">Available Credits</p>
                </div>
                <div className="bg-card border border-border/50 rounded-xl p-4 text-center">
                  <p className="text-2xl font-bold">${lifetimeEarnedDollars.toFixed(2)}</p>
                  <p className="text-[11px] text-muted-foreground mt-1">Lifetime Earned</p>
                </div>
              </div>
            </div>
          )}
