/**
 * EatsLanding - Food delivery hub page with ordering flow
 * Premium glassmorphism style matching the ZIVO super-app
 */
import { useState } from "react";
import { Star, Clock, ArrowRight, Truck, ShoppingCart, Search, MapPin, UtensilsCrossed, Plus, Minus, ArrowLeft, CheckCircle, CreditCard, Package, Timer } from "lucide-react";
...
export default function EatsLanding() {
  ...
  const [step, setStep] = useState<"browse" | "restaurant" | "cart" | "checkout">("browse");
      ...
      <AnimatePresence mode="wait">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                  {filtered.map((restaurant, i) => (
                    <motion.div key={restaurant.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.08 }}>
                      <button
                        onClick={() => { setSelectedRestaurant(restaurant.id); setStep("restaurant"); }}
                        className="group block w-full text-left rounded-2xl bg-card border border-border/40 overflow-hidden hover:shadow-lg hover:shadow-primary/5 hover:-translate-y-1.5 transition-all duration-200 touch-manipulation active:scale-[0.99]"
                      >
          ...
          <motion.div key="restaurant" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="min-h-screen pb-32">
            ...
            <div className="relative h-48 overflow-hidden">
              ...
              <div className="absolute bottom-4 left-4 flex items-center gap-2">
                {currentRestaurant.freeDelivery && (
              ...
              {currentRestaurant.menu.map((item, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flex items-center gap-4 p-4 rounded-2xl bg-card border border-border/40 hover:border-primary/20 transition-all"
                  >
                    <div className="flex-1">
        ...
        {step === "cart" && (
          <motion.div key="cart" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="min-h-screen pb-32">
            ...
            {cart.length > 0 && (
              <div className="fixed bottom-0 left-0 right-0 z-40 p-4 bg-background/95 backdrop-blur-2xl border-t border-border/30 safe-area-bottom">
                <Button onClick={() => { setStep("checkout"); toast.success("Proceeding to checkout!"); }} className="w-full h-14 text-base font-bold gap-3 rounded-2xl bg-gradient-to-r from-primary to-emerald-500 text-primary-foreground shadow-lg shadow-primary/25 active:scale-[0.98]">
                  <CreditCard className="w-5 h-5" />
                  Checkout · ${(cartTotal + deliveryFee + serviceFee).toFixed(2)}
                </Button>
              </div>
            )}
          </motion.div>
          ...
          <motion.div key="checkout" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="min-h-screen pb-32">
            ...
            <div className="px-4 py-6 max-w-lg mx-auto space-y-5">
              ...
              <div className="rounded-2xl bg-card border border-border/40 p-4 space-y-2">
                ...
                <div className="flex justify-between pt-3 border-t border-border/30 mt-3">
                  <span className="font-bold">Total</span>
                  <span className="font-bold text-primary">${(cartTotal + deliveryFee + serviceFee).toFixed(2)}</span>
                </div>
              </div>
              ...
              <Button onClick={() => { toast.success("Order placed! 🎉"); setCart([]); setStep("browse"); }} className="w-full h-14 text-base font-bold gap-3 rounded-2xl bg-gradient-to-r from-primary to-emerald-500 text-primary-foreground shadow-lg shadow-primary/25 active:scale-[0.98]" disabled={!deliveryAddress.trim()}>
